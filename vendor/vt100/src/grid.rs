...
impl Grid {
    ...
    pub fn visible_rows(&self) -> impl Iterator<Item = &crate::row::Row> {
        // PATCHED for sshub: backport of the 0.16 saturating fix.
        // Original 0.15.2 did `rows_len - self.scrollback_offset` unchecked,
        // panicking on integer underflow once the offset exceeded the screen
        // height. Replacing the bounded chain with `.take(rows_len)` lets
        // scrollback peek into older rows than fit on one screen without
