...
- **Runtime appearance boundary** — the [theme system](themes.md) supplies app chrome and the `pty_background` / `pty_foreground` pair for the embedded grid. Remote-selected colors are preserved; session transparency is independently controlled by `appearance.transparent_session_background`.
