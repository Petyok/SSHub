...
- **`parser.rs`** — `vt100::Parser` with 10k scrollback. It answers the supported terminal queries that `vt100` does not implement: cursor position (`CSI 6 n`) from the emulator grid, device status (`CSI 5 n`), and primary device attributes (`CSI c`). Kitty keyboard and secondary-attributes queries remain unanswered to advertise unsupported features. Replies are bounded to 1 KiB per drain. A `clear_buffer()` helper exists to wipe the buffer post-auth but is currently dead code (never called); `-v` handshake noise is siphoned off via the stderr FIFO into `debug_log`, not the parser buffer. **`keys.rs`** encodes crossterm keys to xterm bytes; **`render.rs`** draws header/body/footer and the copy toast.
- **PTY writes** — `PtyRuntime` sends keystrokes, pasted text, auto-typed secrets, and query replies through a dedicated writer thread (`src/session/pty.rs`) backed by a bounded 64-item synchronous queue. This keeps a remote that stops reading from blocking the 50 ms frame loop; ordering is preserved and a full queue drops writes because the remote is not making progress.
