...
|---|---|---|---|
| Unit | `cargo test` | `src/**`, esp. `src/app/tests/` (11 files) | `MockResolver` (HashMap-backed `HostResolver`), `LauncherStore::open_in_memory()`, `NoopPasswordStore` |
...
| E2E | `cargo test --test e2e` | `tests/e2e/` (16 scenario modules) | Build `App` directly via `App::new_with_deps(AppDeps{…})` and inject `crossterm::event::KeyEvent`s into `app.handle_key()`; assert on `app.mode` and store contents |
...
- `tests/fixtures/config.toml` — appearance defaults.
...
n/SFTP logic → `src/app/tests/session.rs`, `src/app/tests/sftp.rs`; keep SFTP logic in the pure `src/sftp/model.rs` where possible.
