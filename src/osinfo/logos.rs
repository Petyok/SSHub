...
// ---------------------------------------------------------------------------
// Large, full-colour logos (fastfetch art) for the zoomed host-detail panel.
// Regenerated by scripts/gen_os_logos.py into assets/os_logos_large.json.
// ---------------------------------------------------------------------------

/// A colour as stored in `os_logos_large.json`: either an ANSI 16-colour index
/// (0-15, so the terminal palette/theme still applies) or a truecolour
/// `[r, g, b]`.
#[derive(Deserialize)]
#[serde(untagged)]
enum RawColor {
...
impl RawColor {
    fn to_color(&self) -> Color {
        match self {
            RawColor::Index(i) => ansi_index_color(*i),
...
static LARGE_LOGOS: OnceLock<HashMap<&'static str, OsLogo>> = OnceLock::new();

fn large_logos() -> &'static HashMap<&'static str, OsLogo> {
    LARGE_LOGOS.get_or_init(|| {
        // Shape: { "<id>": [ [ [text, colour], ... spans ], ...lines... ] }
        type RawLine = Vec<(String, RawColor)>;
        let raw: HashMap<String, Vec<RawLine>> =
            serde_json::from_str(include_str!("../../assets/os_logos_large.json"))
                .expect("assets/os_logos_large.json is malformed");
        let mut map = HashMap::with_capacity(CANONICAL_IDS.len());
        for &id in CANONICAL_IDS.iter() {
            if let Some(lines) = raw.get(id) {
                let lines = lines
                    .iter()
                    .map(|line| {
                        OsLogoLine(
                            line.iter()
                                .map(|(text, c)| OsLogoSpan {
                                    text: text.clone(),
                                    style: Style::default().fg(c.to_color()),
                                })
                                .collect(),
                        )
                    })
                    .collect();
                map.insert(id, OsLogo { id, lines });
            }
        }
        map
    })
...
/// The large, full-colour logo (fastfetch art) for the zoomed host-detail
/// panel. `None` for ids with no large art.
pub fn large_logo_for(id: &str) -> Option<&'static OsLogo> {
    large_logos().get(id)
}
...
mod tests {
    ...
    #[test]
    fn every_canonical_id_has_a_large_logo() {
        for &id in CANONICAL_IDS.iter() {
            let logo = large_logo_for(id).unwrap_or_else(|| panic!("missing large logo for {id}"));
            assert_eq!(logo.id, id);
            assert!(!logo.lines.is_empty(), "{id} large logo is empty");
        }
        // The macOS logo is the multi-colour rainbow apple.
        let macos = large_logo_for("macos").unwrap();
        let colors: std::collections::HashSet<_> = macos
            .lines
            .iter()
            .flat_map(|l| l.0.iter().map(|s| s.style.fg))
            .collect();
        assert!(colors.len() >= 3, "macos large logo should be multi-colour");
    }
}
