//! Searchable host picker for pushing an identity's key with `ssh-copy-id`.
//!
//! Opened from the identities tab: pick a saved host to receive the highlighted
//! identity's public key. Mirrors the "new session tab" picker layout.
...
pub fn render(frame: &mut Frame, app: &App) {
    let Some(picker) = app.copyid_picker.as_ref() else {
        return;
    };
    let matches = app.copyid_host_matches();
    ...
    frame.render_widget(
        ratatui::widgets::Block::default()
            ...
            .title(Span::styled(" copy ssh id to host ", theme::heading()))
